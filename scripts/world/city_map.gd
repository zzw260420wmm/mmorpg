...
func _create_interactables() -> void:
	...
	_add_interactable({
		...
		"size": Vector2(60, 28),
		"lines": {
			"default": ["地铁口的风带着潮气。广告灯箱亮着，像在提醒你：明天还要早起。"],
			"morning": ["早高峰还没完全涌出来，但闸机旁已经有人排队。"],
			"evening": ["人群从楼梯口不断冒出来。每个人都像刚从很远的地方回来。"],
			"late_night": ["末班车提示音很轻，空气里有雨水和便利店咖啡的味道。"],
		},
	...
	_add_interactable({
		...
		"size": Vector2(72, 28),
		"lines": {
			"default": ["这里被压缩成游戏里的人民广场方向：地铁、人流、商场和招聘广告都从这里散开。", "真实上海里，它像浦西的一枚城市节点。"],
			"morning": ["早上这里的人流很快，像一张巨大的表格自动刷新。"],
			"evening": ["夜里商场灯箱亮起来，广场反而显得更空。"],
		},
	...
	_add_interactable({
		...
		"size": Vector2(60, 30),
		"lines": {
			"default": ["黄浦江西岸是外滩方向。游客拍照，打工人赶路，江风把两种生活吹在一起。"],
			"evening": ["对岸陆家嘴的楼亮起来，像一张很贵的工资单。"],
			"late_night": ["江边冷了很多，灯还亮着，人却少了。"],
		},
	...
	_add_interactable({
		...
		"size": Vector2(72, 28),
		"lines": {
			"default": ["西南方向被压缩成徐家汇一带：商场、写字楼、老小区和换乘站叠在一起。"],
			"afternoon": ["午后的商业街很亮，但你只是在心里算今天能不能多赚一点。"],
		},
	...
	_add_interactable({
		...
		"size": Vector2(76, 28),
		"lines": {
			"default": ["过了黄浦江就是浦东陆家嘴方向。写字楼高得像另一套生活规则。"],
			"morning": ["早高峰的人流从地铁口涌向玻璃楼。你知道这里的钱更多，压力也更贵。"],
			"late_night": ["楼上的灯没有全灭。有人把今天延长成了明天。"],
		},
	...
	_add_interactable({
		...
		"size": Vector2(78, 30),
		"lines": {
			"default": ["菜场摊位很挤，塑料袋、湿地面、青菜和鱼腥味混在一起。这里比便利店便宜，但要自己做饭。"],
			"morning": ["早市最热闹，阿姨们砍价的声音比地铁提示音还密。"],
			"late_night": ["菜场已经收摊，地上只剩水痕和几片菜叶。"],
		},
	...
	_add_interactable({
		...
		"size": Vector2(78, 30),
		"lines": {
			"default": ["社区医院门口贴着流感疫苗和体检通知。队伍不长，但每个人看起来都很累。"],
			"morning": ["上午挂号的人更多，很多人请半天假来看病。"],
			"evening": ["晚间门诊的灯很白，照得人更疲惫。"],
		},
	...
	_add_interactable({
		...
		"size": Vector2(88, 30),
		"lines": {
			"default": ["人才公寓楼下贴着申请条件：学历、社保、工作单位、排队资格。看起来比现在的房间干净，也更远。"],
			"evening": ["窗户一格一格亮起来。每一盏灯后面，都是一个刚下班的人。"],
		},
		"fill_color": Color(0.55, 0.76, 0.95, 0.16),
		"border_color": Color("#c7e7ff"),
	})
	_add_interactable({
		...
		"size": Vector2(78, 30),
		"lines": {
			"default": ["中介门口贴满小纸条：一室户、合租、押一付三、近地铁。每个价格后面都藏着距离。"],
			"late_night": ["门店关了，玻璃上还贴着“拎包入住”。你想了想自己的行李，其实也没几件。"],
		},
		"fill_color": Color(0.86, 0.64, 0.38, 0.18),
		"border_color": Color("#e8c879"),
	})
	_add_interactable({
		...
		"size": Vector2(52, 26),
		"lines": {
			"default": ["小饭馆贴着手写菜单：蛋炒饭18，青菜面16。锅气从门缝里钻出来。"],
			"evening": ["晚饭点坐满了附近打工的人。电视里放着听不清的新闻。"],
			"late_night": ["老板准备收摊，只剩汤锅还冒着一点热气。"],
		},
	...
	_add_interactable({
		...
		"size": Vector2(72, 30),
		"lines": {
			"default": ["写字楼玻璃幕墙把天空切成一块一块。你知道自己白天就在这里消耗掉。"],
			"morning": ["门禁机滴滴响个不停，外卖员和上班族一起挤在门口。"],
			"evening": ["楼上还有很多灯亮着。有人刚下班，也有人刚开始加班。"],
			"late_night": ["保安坐在门口刷短视频。整栋楼安静得只剩空调声。"],
		},
	...
	_add_interactable({
		...
		"size": Vector2(72, 28),
		"lines": {
			"default": ["骑手们把电瓶车停成一排，手机提示音此起彼伏。"],
			"morning": ["站长在门口喊单。早高峰的早餐单来得很密。"],
			"evening": ["晚高峰开始了，雨衣和头盔挂在墙上，像一排疲惫的影子。"],
			"late_night": ["夜宵单还在跳，但街上已经只剩店招和尾灯。"],
		},
	...
	_add_interactable({
		...
		"size": Vector2(70, 28),
		"lines": {
			"default": ["楼上贴着短视频机构的招牌。门口的灯很粉，走廊却有点旧。"],
			"morning": ["前台还在拆快递，样品盒堆到墙边。"],
			"evening": ["直播间的补光灯一盏盏亮起来，像另一种加班。"],
			"late_night": ["里面还有人压低声音复盘数据，粉色灯光照着黑眼圈。"],
		},
